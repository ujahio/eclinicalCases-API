...
import { addFeedback, resetAddFeedbackStatus } from "@/store/slices/student/addFeedbackSlice";
...
const StudentFeedbacks: FunctionComponent<StudentFeedbacksProps> = ({ goNext }) => {
  ...
  const updateFeedback = (option: string, value: string) => {
    const feedbackCopy = feedback.map((item) => (item.key === option ? { ...item, response: value } : item));
  ...
  return (
    <>
      <h6 className="text-1xs sm:text-sm font-bold text-blue uppercase mb-2.5">Feedbacks</h6>
                ...
                feedbackRatings.map((rating, ratingIndex) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() => updateFeedback(id, ratingIndex.toString())}
                    />
                    <div
                    ...
                    <span className="inline-block ml-2 text-xs sm:text-1xs text-dark pr-2">{rating}</span>
                  </label>
                ))}
      ...
      <div className="grid sm:grid-cols-2 grid-cols-1 gap-4">
        ...
        >
          {feedbackState.status === "loading" ? "Loading..." : "Submit feedback"}
        </Button>
      </div>
    </>
  );
