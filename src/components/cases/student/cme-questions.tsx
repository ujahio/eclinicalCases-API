...
const StudentCMEQuestions: FunctionComponent<StudentCMEQuestionsProps> = ({
  ...
  const submitResponseState = useAppSelector((state) => state.submitCaseResponse.status);

  const setAnswer = (index: number, value: number | null) => {
    const newAnswers = [...caseDetails.answers];
    newAnswers[index].correctAnswer = value;
  ...
  return (
    <>
      <h6 className="text-1xs sm:text-sm font-bold text-blue uppercase mb-2.5">CASE MODEL MULTI CHOICE QUESTIONS</h6>
      <ul className="flex flex-col space-y-5 sm:space-y-6 mt-5 mb-5 sm:mb-6">
        {caseDetails.answers.map(({ question, options, correctAnswer }, index) => {
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      checked={isChecked(caseDetails.answers[index].correctAnswer, optionIndex)}
                    />
                    <div
                      className={`h-4 w-4 rounded-full border inline-flex items-center justify-center transition-colors ${
                        isChecked(caseDetails.answers[index].correctAnswer, optionIndex) ? "bg-dark" : "border-grey-400"
                      }`}
                    >
                      {isChecked(caseDetails.answers[index].correctAnswer, optionIndex) && (
                        <svg width="8" viewBox="0 0 18.006 12.373" className="text-white">
                          <path
                    ...
                    <span className="text-grey-300 inline-block ml-2.5">{option}</span>
                  </label>
                );
              })}
            </li>
