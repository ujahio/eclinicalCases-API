...
const DoctorMaterialsAndDeadline: FunctionComponent<DoctorMaterialsAndDeadlineProps> = ({
  ...
  const removeFile = (index: number, isExisting: boolean) => {
    if (!isExisting) {
      const updatedExistingFiles = caseStudy.caseMaterials.filter((_: any, i: any) => i !== index);
        ...
        <ul className="mt-3 space-y-2">
          {caseStudy.caseMaterials?.map((file: File, index: number) => (
            ...
            >
              ...
              <button onClick={() => removeFile(index, false)} className="no-outline h-4 w-4">
          ...
          {prevCaseMaterials.map((file: any, index: number) => (
            ...
            >
              ...
              <button onClick={() => removeFile(index, true)} className="no-outline h-4 w-4">
                <svg width="100%" height="100%" viewBox="0 0 19.799 19.799">
                  <g transform="translate(9.899) rotate(45)">
                    <path d="M14,8H8v6H6V8H0V6H6V0H8V6h6Z" fill="#394a5d" />
                  </g>
