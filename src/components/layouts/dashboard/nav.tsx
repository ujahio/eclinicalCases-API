      ...
      >
        ...
        <ul className="h-full hidden sm:flex items-center space-x-6.25 md:space-x-8">
          {navLinks.map(({ label, path }, index) => (
            <li className="h-full relative items-center inline-flex" key={index}>
              <Link
                href={path}
        ...
        <div className="flex items-center cursor-pointer" role="button">
          ...
          <span className="text-dark text-sm inline-block mx-2.5 font-medium">
            {userInfo?.user?.roles === "teacher" ? userInfo?.user?.email : userInfo?.user?.email}
          </span>
          <svg width="10.591" height="6" viewBox="0 0 10.591 6" className="mt-0.5 caret-icon">
            <path
