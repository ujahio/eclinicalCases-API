...
const Modal: FC<ModalProps> = ({ show, toggle, size = "md", children, student }) => {
  let addedClasses = "";
  ...
  return (
    <div
      className={`bg-dark bg-opacity-70 fixed top-0 left-0 right-0 bottom-0 z-50 transform duration-200 ease-in-out flex justify-center items-start ${
        show ? "opacity-1 pointer-events-auto scale-100" : "pointer-events-none opacity-0 scale-95"
      }`}
      style={{ backdropFilter: "blur(3px)" }}
    >
      <div className={`bg-white w-11/12 rounded-sm p-6 sm:p-7 md:p-8 relative ${addedClasses}`}>
        <button
