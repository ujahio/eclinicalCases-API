...
const Signup = ({ handleSignUp }: { handleSignUp: any }) => {
  ...
  const handleCaptureProfessionalDetails = (professionalDetails: ProfessionalDetailsProps) => {
        ...
        <div className="w-11/12 max-w-200 bg-white py-10 px-6 sm:p-10 md:p-12.5 border border-grey-border rounded-sm mx-auto">
          ...
          <div className="mt-8">
            ...
            {isActive("personal_details") && (
              <SignUpForm switchByKey={switchByKey} caputurePersonalDetails={handleCapturePersonalDetails} />
            )}
            {isActive("professional_details") && (
              <ProfessionalDetailsForm
                switchByKey={switchByKey}
                handleCaptureProfessionalDetails={handleCaptureProfessionalDetails}
              />
            )}
            {isActive("review_and_confirm") && (
              <ReviewAndConfirm personalDetailsInfo={personalDetailsInfo} handleSignUp={handleSignUp} />
            )}
          </div>
        </div>
      </div>
