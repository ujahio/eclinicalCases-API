...
const DoctorDashboard = () => {
  ...
  const ongoingCasesState = useAppSelector((state) => state.onGoingCase.cases);
  const cases =
  ...
  return (
    <AdminLayout>
      <ResponseFeedbackModal show={showModal} toggle={toggleModal} student={"selectedStudent"} caseId={caseId} />
      <div>
        ...
        <div className="mt-14">
          <h5 className="text-1sm sm:text-base text-dark uppercase mb-3.75">ONGOING CASE STUDY</h5>
          ...
          <div className="w-full px-6 py-8 sm:px-8 sm:py-10 md:px-10 md:py-12 ongoing-case flex flex-col sm:flex-row sm:items-center justify-between flex-wrap text-white rounded-sm">
            {ongoingCasesState?.data?.length !== 0 ? (
              <div className="flex flex-col md:mr-4 mb-4">
                ...
                <h5 className="font-bold text-base mt-3.75 mb-2.5">{ongoingCasesState?.data[0]?.caseTopic}</h5>
                <p className="text-1sm text-sm max-w-lg mb-5">
                  Learn how patients with a serious infection can be managed in outpatient settings with the help of an
                  OPAT service.
                </p>
                <div>
                  <span className="inline-block text-1xs">
                    <b>Created:</b> {formatDate(ongoingCasesState?.data[0]?.createdAt)}
                  </span>
                  <span className="block sm:inline-block text-1xs sm:ml-8">
                    <b>Deadline:</b> {formatDate(ongoingCasesState?.data[0]?.caseDeadline)}
                  </span>
                </div>
                <div className="flex mt-6">
                  <button
                    onClick={() => {
                      setCaseId(ongoingCasesState?.data[0].id);
                      setShowModal(true);
                    }}
                    className=" bg-neutral-200 bg-opacity-20 rounded-sm w-24 text-center py-2 text-xs"
                  >
                    {ongoingCasesState?.data[0]?.totalFeedbacks || 0} Feedbacks
                  </button>
                  <div className=" bg-neutral-200 bg-opacity-20 rounded-sm w-24 text-center py-2 ml-3 text-xs">
                    {ongoingCasesState?.data[0]?.totalResponses || 0} Responses
                  </div>
                </div>
              </div>
            ) : (
            ...
            <div className="min-w-min inline-block">
              <div className="flex">
                <Link href="/doctor/case-studies/create">
                  <Button type="button" size="md" btnStyle="white" className="mr-1">
                    Create New Case
                  </Button>
                </Link>
                <div>
                  ...
                  <AppDropdown>
                    ...
                    <AppDropdownItem>Create From an Existing Case</AppDropdownItem>
                  </AppDropdown>
                </div>
              </div>
            </div>
        ...
        <div className="mt-14">
          <div className="flex justify-between items-center mb-3.75">
            <h5 className="text-1sm sm:text-base text-dark uppercase">RECENT CASE STUDIES</h5>
            {cases?.length > 0 ? (
              <Link
                href="/doctor/cases"
