...
const CheckBox: FunctionComponent<CheckBoxProps> = ({ children, onChange, isChecked = false, name }) => {
  return (
    <label htmlFor={name} className="cursor-pointer inline-flex items-center">
      <input type="checkbox" className="hidden" onChange={(e) => onChange(e.target.checked)} name={name} id={name} />
      <span
        role="button"
        className={`outline-none focus:outline-none border-0.375 rounded-sm h-3.75 w-3.75 inline-flex items-center justify-center transition-colors duration-150 ${
          isChecked ? "bg-primary-300 border-primary-300" : "bg-neutral-200 border-grey-200"
        }`}
      >
        <svg
          width="80%"
