...
const CaseCard: FunctionComponent<CaseCardProps> = ({
  case: { _id, title, description, deadline, created, feedback, responses, caseStatus },
  handleDeleteCase,
}) => {
  ...
  return (
    ...
    >
      {caseStatus === "draft" ? (
        <div className="py-2 px-3.5 bg-slate-400 absolute right-3 top-3 capitalize text-white">{caseStatus}</div>
      ) : null}
      ...
      {feedback && responses && (
        <div className="flex mt-4">
          <div className=" bg-neutral-200  rounded-sm w-24 text-center py-2 text-xs">{feedback}</div>
          <div className=" bg-neutral-200  rounded-sm w-24 text-center py-2 ml-3 text-xs">{responses}</div>
        </div>
      )}
      {caseStatus === "draft" ? (
