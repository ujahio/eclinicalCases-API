...
const AppDropdown = ({ children, className, closeOnClick = true, position = "right" }: AppDropdownProps) => {
  ...
  useEffect(() => {
    if (dropdownEl.current) {
      ...
      if (dropdownParent) {
        ...
        return () => dropdownParent.removeEventListener("click", toggleFunction);
      }
    }
  }, [show]);
  ...
  const stateClasses = () => {
    return show ? "opacity-1 translate-y-1" : "opacity-0 translate-y-3 pointer-events-none";
  };
