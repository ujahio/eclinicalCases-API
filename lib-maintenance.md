# Library Maintanance

1. react: As of Jan, 5th, 2025, version is locked to v18 as "jsx-email" is not compatible with React v19.
2. react-dom: As of Jan, 5th, 2025, version is locked to v18 as "jsx-email" is not compatible with react-dom v19.
3. @types/react: As of Jan, 5th, 2025, version is locked to v18 as react, react-dom cannot be upgraded to v19.
4. @types/react-dom: As of Jan, 5th, 2025, version is locked to v18 as react, react-dom cannot be upgraded to v19.
5. sass: locked to v1.77.3 as v1.88+ introduces new breaking changes with new apis.
6. sst: locked to 3.2.73 as v3.2.73+ does not deploy the app correctly.
